  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        ...
        const getCategoryName = response.data.map((name) => name.category_name);
      ...
      try {
        ...
        if (selectedStatus && filterEventType && filterCategory) {
          response = await api.get(
            `/event/filter/${selectedStatus}/${filterEventType}/${filterCategory}`
          );
          setEvent(response.data);
        } else if (filterEventType && filterCategory) {
          response = await api.get(
            `/event/filter/event_type?event_type=${filterEventType}&event_category=${filterCategory}`
          );
          setEvent(response.data);
        } else if (selectedStatus && filterCategory) {
          response = await api.get(
            `/event/filter/${selectedStatus}/${filterCategory}`
          );
          setEvent(response.data);
        } else if (selectedStatus && filterEventType) {
          response = await api.get(
            `/event/filter/status/${selectedStatus}/event-type/${filterEventType}`
          );
          setEvent(response.data);
          ...
          response = await api.get(
            `/event/filter/event-type/${filterEventType}`
          );
          setEvent(response.data);
        }
      } catch (error) {
  ...
  const handleFilterType = async (selectedType) => {
    try {
      const response = await api.get(
        `/event/filter/event-type/${selectedType}`
      );
      setEvent(response.data);
    } catch (error) {
      console.log(error);
  ...
  return (
    <div className="">
      <div className="flex justify-between p-5 items-center">
        <h1
          className="font-bold  lg:text-4xl text-xl text-[#243e63] cursor-pointer"
      ...
      <div className="flex px-5 md:px-0 lg:flex-row flex-col">
        <div className="lg:hidden flex justify-evenly py-5 items-center">
          ...
          <DropdownCategory
            options={[{ label: "Status", links: ["Upcoming", "Completed"] }]}
