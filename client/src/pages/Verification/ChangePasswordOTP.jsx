...
function ChangePasswordOTP() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      const response = await api.post("/auth/confirm-otp", values);
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      const response = await api.post("/auth/resend-otp", values);
      console.log(response.data);
      if (response.data.status === "success") {
        setTimeout(() => {
          setSuccessMessage(response.data.message);
      ...
      <div className="flex flex-col items-center justify-center ">
        ...
        <div className="w-[350px] sm:mx-auto sm:w-full sm:max-w-md px-4 py-4 mt-6 overflow-hidden bg-white p-4 rounded-lg shadow-md">
          ...
          <form onSubmit={handleVerify} className="flex flex-col gap-4">
            ...
            {/* </div> */}
            <button
              className={`bg-gradient-to-r from-[#f87a58] via-[#f7426f] to-[#f87a58] text-white px-4 py-2 mt-2 rounded-md hover:from-[#f7426f] hover:to-[#f7426f] hover:via-[#f87a58] focus:outline-none focus:bg-indigo-600
