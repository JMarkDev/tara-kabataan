        ...
        <div className='bg-white shadow-lg'>
            ...
            <div className="grid grid-cols-3 gap-2 justify-center mx-auto items-center">
                {sampleData.map((item, index) => (
                    <div key={`color-${index}`} className="flex justify-start items-center mx-auto mb-5">
                        <div className="w-6 h-6 mr-2" style={{ backgroundColor: COLORS[index % COLORS.length] }}></div>
                        <p className='flex cursor-pointer font-bold justify-center items-center mx-auto'>{item.gender}</p>
                    </div>
                ))}
            </div>
        </div>
