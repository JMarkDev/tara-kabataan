...
const Chatbot = ({ setOpenChat, openChat }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if(userQuery === '') return
      const response = await api.post('/chatbot/query', { user_query:userQuery })
      const updatedConversation = [...conversation, { user_query: userQuery, bot_response: response.data.data }]
    ...
    return (
    <div className="bg-[#f2f2f2] h-[420px] transition-all shadow-2xl drop-shadow-xl w-[350px] fixed  bottom-[90px] rounded-lg right-5">
              ...
              <div className="h-[280px] pb-2 overflow-y-auto " ref={chatContainer}>
                  <div className="flex flex-col gap-2 mt-7 mx-3">
                    <div className='flex gap-2'>
                      ...
                      <p className="text-sm rounded-lg p-2 mr-8 bg-gray-300 py-3 transition-all ">Welcome to out Chatbot? How can I assist you today?</p>
                    </div>
                    {
                          ...
                          <div key={index}>
                            ...
                            <div className="flex gap-2">
                              <img src={ChatbotIMG} alt="chatbot" className="w-7 h-7 rounded-full"/>
                                ...
                                { lastItem && isLoading ? <Loading /> :                       
                                  <p className="text-sm rounded-lg p-2 mr-8 bg-gray-300">{convo.bot_response}</p> 
                                }
                                </div>
                            </div>
                          </div>
                ...
                <div className="absolute rounded-b-lg bg-[#f2f2f2] left-0 bottom-0 w-full px-3">
                  ...
                  <input type="text" className="relative w-full mb-2 p-2 rounded-full text-sm focus:outline-none border border-indigo-400 focus:border-[#6415ff]" placeholder="Type a message"
                  value={userQuery}
                  onChange={(e) => setUserQuery(e.target.value)}
                  />
                  </div>
                  </form>
                  <p className="text-sm p-1 flex justify-center text-gray-600">@tarakabataan{currentYear}</p>
                </div>
            </div>
  )
}
